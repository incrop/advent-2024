...
func ascii(selectedDay int, dayStars [26]int) []string {
	return applyStyles(
		...
		`        ggg                ¢  $ r       rrrGGOGG `,
		`       |'''))   |        | o  * : '.  .'──┘>>o<<|`,
		`        BB  BBBB          r rR   i p   pp GGRGOG `,
		`.------'.-((---.'------. |. :|\| ~ _'' O> >>@<o<|`,
		`  RRRRR BBBBBBBBB           r RRpppppppi GRGG$GB `,
		`| .---_ '------'_  .~' | |  .|\|\ / \ /~ >@<<*<O|`,
		` RRRRRRRBBBBBBBBBB R      BBBR  BBBBBBBBGBGGOGGO `,
		`|/ / /\||      | )/~\  | |___|\|________>O>>o>>o|`,
		` RRRRR $BBBBBBBBB  R        bbbbb  6666666666    `,
		`|/ | \ *|      |// / \ | |  ----@  _|%%%=%%|_   |`,
	)
}
...
var purple lipgloss.Style = lipgloss.NewStyle().Foreground(lipgloss.Color("#9900ff"))
